...
<body>
    ...
    $dato_skill= $skillController->ListarSkill($id);
    ...
    ?>
    <?php 
    foreach($dato_skill as $row_skill){
        $id_skill=$row_skill['IdSkills'];
        $skill= $skillController->ListarSkillporid($id_skill);
        foreach($skill as $row_skillporid){
            echo('
          
            <input class="class_id_persona d-none" value="'.$row['IdPersona'].'">
            <div class="col-md-3">
                <p class="t h5"><i class="fas fa-check  me-2 text-dark"></i>'.$row_skillporid['Descripcion'].'</p>
                <div class="form-check">
                    <input class="searchType" type="checkbox" value="'.$row_skillporid['IdSkills'].'" >
                    <label class="form-check-label" for="flexCheckDefault">
                        Validar skill
                    </label>
                </div>
                <div class="checkProfesor"><p></p></div>
            </div>
